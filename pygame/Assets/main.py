...
BULLECT_FIRE_SOUND = pygame.mixer.Sound(os.path.join('Assets','Gun+Silencer.mp3'))
WINNER_SOUND = pygame.mixer.Sound(os.path.join('Assets', 'win.wav'))

HEALTH_FONT = pygame.font.SysFont('comicsans', 40)
WINNER_FONT= pygame.font.SysFont('comicsans', 100)

FPS = 60
VEL = 7 #speed when pressed
BULLET_VEL = 15
MAX_BULLETS = 3
...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    ...
    red_health_text = HEALTH_FONT.render(
        "Health: " + str(red_health), 1, ORANGE)
    yellow_health_text = HEALTH_FONT.render(
        "Health: " + str(yellow_health), 1, ORANGE)
...
def yellow_movement_handle(keys_pressed,yellow):
    ...
    if keys_pressed[pygame.K_w] and yellow.y -VEL > 0: #up key
            yellow.y -= VEL 
    if keys_pressed[pygame.K_s] and yellow.y + VEL + yellow.height < HEIGHT - 14: #down key
            yellow.y += VEL
def red_movement_handle(keys_pressed,red):
    ...
    if keys_pressed[pygame.K_UP] and red.y -VEL > 0: #up key
            red.y -= VEL 
    if keys_pressed[pygame.K_DOWN] and red.y + VEL + red.height < HEIGHT - 14: #down key
            red.y += VEL
...
def main():
    ...
    yellow = pygame.Rect(50,350, SPACESHIP_WIDTH,SPACESHIP_HEIGHT)
